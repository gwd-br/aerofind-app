  ...
  <Dialog :open="open" @update:open="onOpenChange">
    <DialogContent class="sm:max-w-md" :show-close-button="true">
      <DialogHeader>
        ...
        <DialogDescription>
          Preencha os dados do setor. Ele será vinculado ao evento atual.
        </DialogDescription>
      </DialogHeader>
      <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
        <div class="flex flex-col gap-2">
          ...
          <Input
            ...
            required
          />
          <p v-if="errors.nome_setor" class="text-red-500 text-sm">{{ errors.nome_setor }}</p>
        </div>
        <div class="flex flex-col gap-2">
...
watch(
  ...
  (open) => {
    if (open) {
      form.eventos_fk = props.eventoId;
      form.nome_setor = "";
      form.categoria = null;
      form.area = null;
      form.entrada = null;
      form.inventario = null;
      form.facial = false;
...
function validate(): boolean {
  errors.nome_setor = !form.nome_setor?.trim() ? "Informe o nome do setor." : undefined;
