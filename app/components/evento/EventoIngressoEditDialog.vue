  ...
  <Dialog :open="open" @update:open="onOpenChange">
    <DialogContent class="sm:max-w-lg" :show-close-button="true">
      <DialogHeader>
        ...
        <DialogDescription> Altere os dados do ingresso. </DialogDescription>
      </DialogHeader>
      <form
        v-if="ingresso"
...
const { update } = useIngressoService();
...
watch(
  () => [props.open, props.ingresso] as const,
  ([open, ingresso]) => {
    if (open && ingresso) {
      form.barcode = ingresso.barcode ?? null;
      form.num_pedido = ingresso.num_pedido ?? null;
      form.tipo_ingresso = ingresso.tipo_ingresso ?? null;
      form.tipo_entrega = ingresso.tipo_entrega ?? null;
      form.tipo_venda = ingresso.tipo_venda ?? null;
      form.status = ingresso.status ?? null;
      form.setores_fk = ingresso.setores_fk ?? null;
      form.evento_fk = ingresso.evento_fk ?? props.eventoId;
      form.dt_compra = ingresso.dt_compra ?? null;
      form.dt_venda = ingresso.dt_venda ?? null;
      form.enviado_acesso = ingresso.enviado_acesso ?? false;
      form.url_onboarding = ingresso.url_onboarding ?? null;
    }
  },
  { immediate: true },
);
...
async function handleSubmit() {
  ...
  try {
    await update(props.ingresso.id, {
      ...
      evento_fk: form.evento_fk ?? props.eventoId,
